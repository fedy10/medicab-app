...
import { MessageCircle, Send, X, Edit2, Trash2, MoreVertical, Shield, Check, Paperclip, Download, FileText, Image as ImageIcon } from 'lucide-react';
...
interface DoctorAdminChatProps {
  userId: string;
  userName: string;
}

export function DoctorAdminChat({ userId, userName }: DoctorAdminChatProps) {
    ...
    try {
      ...
      if (admin) {
        setAdminInfo({
          ...
          avatar: `${admin.nom[0]}${admin.prenom[0]}`,
        });
      }
      
      if (currentUser) {
        setUserRole(currentUser.role === 'admin' ? 'Administrateur' : currentUser.role === 'medecin' ? 'Médecin' : 'Utilisateur');
      }
    } catch (error) {
  ...
  const fetchMessages = () => {
    try {
      ...
      if (admin) {
        const conversationKey = `messages_${[userId, admin.id].sort().join('_')}`;
  ...
  const handleSendMessage = () => {
    if (!message.trim() || !adminInfo) return;
    ...
    const conversationKey = `messages_${[userId, adminInfo.id].sort().join('_')}`;
    console.log('💬 [DoctorAdminChat] Sending message with key:', conversationKey);
    console.log('💬 [DoctorAdminChat] Doctor ID:', userId);
    console.log('💬 [DoctorAdminChat] Admin ID:', adminInfo.id);
    console.log('💬 [DoctorAdminChat] Message:', newMessage);
    
    const updatedMessages = [...messages, newMessage];
    setMessages(updatedMessages);
    localStorage.setItem(conversationKey, JSON.stringify(updatedMessages));
    console.log('💬 [DoctorAdminChat] Messages saved:', updatedMessages);
    setMessage('');
  };
  ...
  const saveEditedMessage = () => {
    if (!editingContent.trim() || !adminInfo) return;
  ...
  const markMessagesAsRead = () => {
    ...
    try {
      const conversationKey = `messages_${[userId, adminInfo.id].sort().join('_')}`;
      console.log('✅ [DoctorAdminChat] Marking messages as read for key:', conversationKey);
      const storedMessages = localStorage.getItem(conversationKey);
      ...
      if (storedMessages) {
        ...
        setMessages(updatedMessages);
        localStorage.setItem(conversationKey, JSON.stringify(updatedMessages));
        console.log('✅ [DoctorAdminChat] Messages marked as read');
      }
    } catch (error) {
      ...
      <div className="bg-gradient-to-r from-purple-500 to-pink-600 p-4 text-white">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm">Chat avec Admin</h3>
              {adminInfo && (
        ...
        {messages.length === 0 ? (
          <div className="flex items-center justify-center h-full text-gray-500">
            <div className="text-center">
              ...
              <p className="text-xs mt-1">Commencez la conversation avec l'admin</p>
            </div>
          </div>
        ) : (
          messages.map((msg) => {
